...
from rememberly.profiles.models import Profile
...
#class Profile(models.Model):
    """
    A profile is a profile known by a user who may or may not themselves
    be a user.
    """
...
#    added = models.DateField(default=datetime.date.today)
    ...
    # the user(s) this profile correspond to
#    users = models.ManyToManyField(User)
    
#    def __unicode__(self):
#        return "%s (%s's profile)" % (self.email, self.user)
...
class JoinInvitationManager(models.Manager):
    ...
    def send_invitation(self, from_user, to_email, message):
        profile, created = Profile.objects.get_or_create(email=to_email, invited_by=from_user)
        ...
        return self.create(from_user=from_user, profile=profile, message=message, status="2", confirmation_key=confirmation_key)


class JoinInvitation(models.Model):
    """
    A join invite is an invitation to join the site from a user to a
    profile who is not known to be a user.
    """
...
if EmailAddress:
    def new_user(sender, instance, **kwargs):
        if instance.verified:
            for join_invitation in JoinInvitation.objects.filter(profile__email=instance.email):
            ...
            for profile in Profile.objects.filter(email=instance.email):
                profile.users.add(instance.user)
                # @@@ send notification
