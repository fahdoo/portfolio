...
def create_profile(request, form_class=None, success_url=None,
    ...
    **Template:**
    ...
    if success_url is None:
        success_url = reverse('my_profile',
...
def profile_detail(request, username, public_profile_field=None,
    ...
    **Template:**
    ...
    questions = Question.objects.filter(user = user).order_by('-created')[:10]
    answers = Answer.objects.filter(user = user).order_by('-created')[:10]
    payload = {'profile': profile_obj, 'questions':questions, 'answers':answers}
        
    return render_to_response(template_name, payload, context_instance=context)
